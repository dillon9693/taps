...
function TapsLogoSmooth({ width = 32, height = 32 }: TapsLogoProps) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`beerGradient-${id}`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" style={{ stopColor: '#FFD700', stopOpacity: 1 }} />
          <stop offset="100%" style={{ stopColor: '#FFA000', stopOpacity: 1 }} />
        </linearGradient>
        <linearGradient id={`glassGradient-${id}`} x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      {/* Glass highlight */}
      <path d="M11 6c0 0 2 0 2 0v16c0 0-2 0-2 0z" fill={`url(#glassGradient-${id})`} />
      {/* Bubbles with subtle glow */}
      <g fill="#FFE082" opacity="0.8" filter={`url(#glow-${id})`}>
        <circle cx="14" cy="14" r="0.8">
          <animate attributeName="cy" from="14" to="12" dur="3s" repeatCount="indefinite" />
        </circle>
        <circle cx="18" cy="16" r="0.8">
          <animate attributeName="cy" from="16" to="14" dur="2.5s" repeatCount="indefinite" />
        </circle>
        <circle cx="16" cy="18" r="0.8">
          <animate attributeName="cy" from="18" to="16" dur="2.8s" repeatCount="indefinite" />
        </circle>
      </g>
    </svg>
  );
